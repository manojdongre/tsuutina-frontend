...
interface HistoryItem {
  ...
  yearsFrom: number;
  yearsUpto: number;
  imageUrl: string; // Change from image to imageUrl
  description: string;
  index: number;
}
...
function HistoryManagementPage(): React.JSX.Element {
  ...
  const fetchHistoryItems = async () => {
    try {
      const response: ApiResponse<HistoryItem[]> = await apiService.getHistoryItems();
      if (response) {
        setHistoryItems(response.data);
      } else {
        setError(response || 'Failed to fetch history items');
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      let response: ApiResponse<HistoryItem>;
      if (selectedItem) {
        response = await apiService.updateHistoryItem(selectedItem._id!, selectedItem);
      } else {
        ...
        <Card key={item._id} sx={{ mb: 2, p: 2 }}>
          ...
          <Stack direction="row" gap={2} mt={2}>
            {/* <Switch
              ...
              onChange={async (e) => {
                const updatedItem = { ...item, isCurrentlyVisible: e.target.checked };
                const response = await apiService.updateHistoryItem(item._id!, updatedItem);
                if (response.success) {
                  fetchHistoryItems();
                } else {
